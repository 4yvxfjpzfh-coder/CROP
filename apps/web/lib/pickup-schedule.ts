...
export const PICKUP_DAY_LABELS = [
  "Domingo",
  "Lunes",
  "Martes",
  "Miércoles",
...
/**
 * Fecha (año/mes/día en hora de Costa Rica) del próximo día "pickupDay" a
 * partir de "now". Si "now" ya es ese día pero pasada la hora de cierre de
 * la feria, se toma la semana siguiente.
 */
function nextPickupDayCR(
  now: Date,
  pickupDay: number,
): { year: number; month: number; day: number } {
  const crShifted = new Date(now.getTime() - CR_UTC_OFFSET_HOURS * 3600 * 1000);
  const dow = crShifted.getUTCDay(); // 0=domingo ... 6=sábado
  const crHour = crShifted.getUTCHours();

  let daysUntil = (pickupDay - dow + 7) % 7;
  if (daysUntil === 0 && crHour >= FERIA_CLOSE_HOUR) {
    daysUntil = 7;
  }
...
/**
 * Valida que el slot elegido sea de verdad uno de los del próximo día de
 * feria para ese punto de recogida (nunca se confía en la fecha que manda
 * el cliente sin recalcularla acá) y devuelve la fecha límite real de
 * recogida: la hora elegida + 2.5 horas.
 */
export function resolvePickupDeadline(
  ...
  const valid = getPickupSlots(pickupDay, now);
